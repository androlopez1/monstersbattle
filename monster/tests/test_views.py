...
class MonsterAPITests(MonsterAPISetUp):
    def test_monster_successful_import(self):
        ...
        self.assertEqual(Monster.objects.count(), 11)

    def test_monster_empty_import(self):
        response = self.post_file(self.empty_csv)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data, {"Error": "Insert monsters with valid formats"})
        self.assertEqual(Monster.objects.count(), 0)

    def test_monster_wrong_import(self):
        response = self.post_file(self.wrong_csv)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data, {"Error": "Insert monsters with valid formats"})
        self.assertEqual(Monster.objects.count(), 0)
